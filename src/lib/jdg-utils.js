...
export const getImageMetaByKey = (registry, key) => {
	if (!registry || !key || typeof key !== 'string') return null;
...
// Resolve an array of registry keys to imageMeta objects
export const resolveImageMetaKeys = (keys, registry) => {
	if (!Array.isArray(keys) || !registry) return [];

	return keys
		.filter((key) => typeof key === 'string')
		.map((key) => getImageMetaByKey(registry, key))
		.filter(Boolean);
};
